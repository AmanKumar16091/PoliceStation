...
router.post('/registerAdmin', (req, res) =>{
    ...
    if(password == confirm_password){
        db.query('SELECT email FROM official WHERE email = ?', [email], async (error, result)=>{
            ...
            db.query('INSERT INTO official SET ?', {password : hashedPassword, official_name : official_name, email:email, contact_no:contact_no, post : post}, (error, result)=>{
                if(error){
                    console.log(error)
                }
                else{
...
router.post('/registerVehicle', (req, res) =>{
    ...
    db.query('SELECT vehicle_no FROM vehicle WHERE vehicle_no = ?', [vehicle_no], async (error, result)=>{
        ...
        db.query('INSERT INTO vehicle SET ?', {vehicle_no : vehicle_no, vehicle_type : vehicle_type, description : description, official_id : 1}, (error, result)=>{
            if(error){
                console.log(error)
            }
            else{
...
router.post('/registerWeapon', (req, res) =>{
    ...
    db.query('INSERT INTO weapon SET ?', {weapon_type : weapon_type, quantity : quantity, description : description, no_of_bullets : no_of_bullets, official_id : 1}, (error, result)=>{
        if(error){
            console.log(error)
        }
        else{
...
router.post('/registerCriminal', (req, res) =>{
    ...
    db.query('SELECT aadhar_no FROM criminal_record WHERE aadhar_no = ?', [aadhar_no], async (error, result)=>{
        ...
        db.query('INSERT INTO criminal_record SET ?', {first_name : first_name, middle_name : middle_name, last_name : last_name, criminal_contact_no : criminal_contact_no, description : description, aadhar_no : aadhar_no, fir_no : fir_no}, (error, result)=>{
            if(error){
                console.log(error)
            }
            else{
...
router.post('/registerChallan', (req, res) =>{
    ...
    db.query('INSERT INTO challan SET ?', {dl_number : dl_number, vehicle_type : vehicle_type, vehicle_no : vehicle_no, date_and_time : date_and_time, aadhar_no : aadhar_no, voilation : voilation, fine_amount : fine_amount, payment_status : payment_status, official_id  : 2 }, (error, result)=>{
        if(error){
            console.log(error)
        }
        else{
